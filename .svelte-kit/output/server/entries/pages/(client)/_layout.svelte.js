...
function Navbar($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
			...
			if (link.dropdown) {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`<button${attr_class(`flex items-center gap-1 text-[17px] font-serif transition-colors duration-200 outline-none ${link.dropdown.some((d) => store_get($$store_subs ??= {}, "$page", page).url.pathname === d.path || store_get($$store_subs ??= {}, "$page", page).url.pathname.startsWith(d.path + "/")) ? "font-bold text-emerald-800" : "text-black hover:text-emerald-700"}`)}>${escape_html(link.name)} <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4 mt-0.5 opacity-70 group-hover:rotate-180 transition-transform duration-300"><path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd"></path></svg></button> <div class="absolute left-0 top-full pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300"><div class="bg-white border border-gray-100 shadow-xl rounded-xl py-3 w-48 flex flex-col"><!--[-->`);
				const each_array_1 = ensure_array_like(link.dropdown);
				for (let $$index = 0, $$length = each_array_1.length; $$index < $$length; $$index++) {
					let dropLink = each_array_1[$$index];
					$$renderer.push(`<a${attr("href", dropLink.path)}${attr_class(`px-5 py-2.5 text-[15px] font-serif transition-colors ${store_get($$store_subs ??= {}, "$page", page).url.pathname === dropLink.path || store_get($$store_subs ??= {}, "$page", page).url.pathname.startsWith(dropLink.path + "/") ? "font-bold text-emerald-800 bg-emerald-50" : "text-gray-700 hover:bg-emerald-50 hover:text-emerald-800"}`)}>${escape_html(dropLink.name)}</a>`);
				}

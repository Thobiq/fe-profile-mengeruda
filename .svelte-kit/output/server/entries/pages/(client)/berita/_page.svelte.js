import { C as attr, a as head, c as stringify, i as ensure_array_like, n as attr_style, t as attr_class, w as escape_html } from "../../../../chunks/server.js";
//#region src/lib/components/HeroBerita.svelte
function HeroBerita($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		const beritaTerbaru = [
			{
				title: "Gotong Royong Warga Mengeruda melakukan Perbaikan Sistem Irigasi Sawah",
				img: "/hero-1.jpg",
				slug: "gotong-royong-perbaikan-irigasi"
			},
			{
				title: "Penyaluran Bantuan Langsung Tunai (BLT) Dana Desa Tahap 3 Berjalan Lancar",
				img: "/hero-1.jpg",
				slug: "penyaluran-blt-tahap-3"
			},
			{
				title: "Desa Mengeruda Sukses Gelar Pelatihan Digitalisasi untuk UMKM Lokal",
				img: "/hero-1.jpg",
				slug: "pelatihan-digitalisasi-umkm"
			},
			{
				title: "Persiapan Lomba Desa Tingkat Kabupaten, Warga Hias Lingkungan",
				img: "/hero-1.jpg",
				slug: "persiapan-lomba-desa"
			},
			{
				title: "Mahasiswa KKN Gelar Penyuluhan Kesehatan Stunting di Balai Desa",
				img: "/hero-1.jpg",
				slug: "penyuluhan-kesehatan-stunting"
			}
		];
		let currentIndex = 0;
...
function ListBerita($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		const daftarBerita = Array(6).fill({
			...
			img: "/hero-1.jpg",
			slug: "koordinasi-pemerintah-desa-kkn"
		});
		let searchQuery = "";
		let currentPage = 1;
		let totalPages = 4;
		...
		$$renderer.push(`<!--]--> <button${attr("disabled", false, true)} class="w-10 h-10 flex items-center justify-center rounded border border-gray-300 text-gray-600 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-serif text-lg">></button></div></div></section>`);
	});
}
//#endregion

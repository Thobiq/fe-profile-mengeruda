...
function _layout($$renderer, $$props) {
  ...
  `)}><div class="flex items-center gap-3 px-6 py-8"><div class="w-12 h-12 bg-white rounded-full p-1 flex items-center justify-center shrink-0 shadow-sm overflow-hidden border border-white/20"><img${attr("src", logoUrl)}${attr("alt", `Logo ${stringify(namaDesa)}`)} class="w-full h-full object-contain"/></div> <div><h2 class="font-bold text-[17px] leading-tight">${escape_html(namaDesa)}</h2> <p class="text-[13px] text-white/80 font-medium">Admin Panel</p></div></div> <nav class="flex-1 px-4 space-y-1.5 overflow-y-auto mt-2"><!--[-->`);
		...
		for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
			...
			$$renderer.push(`<a${attr("href", menu.path)}${attr_class(`flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-[15px] transition-colors
            ${currentPath() === menu.path ? "bg-white text-[#006e33]" : "text-white hover:bg-white/10"}
          `)}><span class="shrink-0">${html(menu.icon)}</span> ${escape_html(menu.name)}</a>`);
		}
		$$renderer.push(`<!--]--></nav> <div class="p-4 mb-4"><button class="flex items-center gap-3 px-4 py-3 w-full text-white hover:bg-white/10 rounded-xl font-bold text-[15px] transition-colors"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 rotate-180"><path fill-rule="evenodd" d="M7.5 3.75A1.5 1.5 0 006 5.25v13.5a1.5 1.5 0 001.5 1.5h6a1.5 1.5 0 001.5-1.5V15a.75.75 0 011.5 0v3.75a3 3 0 01-3 3h-6a3 3 0 01-3-3V5.25a3 3 0 013-3h6a3 3 0 013 3V9A.75.75 0 0115 9V5.25a1.5 1.5 0 00-1.5-1.5h-6zm10.72 4.72a.75.75 0 011.06 0l3 3a.75.75 0 010 1.06l-3 3a.75.75 0 11-1.06-1.06l1.72-1.72H9a.75.75 0 010-1.5h10.94l-1.72-1.72a.75.75 0 010-1.06z" clip-rule="evenodd"></path></svg> Logout</button></div></aside> <div class="flex-1 flex flex-col min-w-0"><header class="h-[76px] bg-white border-b border-gray-200 flex items-center justify-between px-6 lg:px-10 shrink-0"><div class="flex items-center gap-4"><button class="lg:hidden p-2 text-gray-600 hover:bg-gray-100 rounded-lg"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"></path></svg></button> <h1 class="text-xl font-bold text-[#006e33]">${escape_html(pageTitle())}</h1></div> <div class="flex items-center gap-3"><span class="font-bold text-[#006e33] hidden sm:block">Admin</span> <div class="w-10 h-10 bg-[#006e33] rounded-full flex items-center justify-center text-white"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><path fill-rule="evenodd" d="M7.5 6a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM3.751 20.105a8.25 8.25 0 0116.498 0 .75.75 0 01-.437.695A18.683 18.683 0 0112 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 01-.437-.695z" clip-rule="evenodd"></path></svg></div></div></header> <main class="flex-1 overflow-x-hidden overflow-y-auto p-6 lg:p-10 relative">`);
		$$renderer.push("<!--[-1-->");
		children($$renderer);
		$$renderer.push(`<!---->`);

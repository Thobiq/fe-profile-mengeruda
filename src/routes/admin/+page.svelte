<script>
  // Data statis untuk kartu dashboard sesuai gambar mockup
...
<svelte:head>
  <title>Dashboard Admin - Desa Mengeruda</title>
</svelte:head>

<!-- Grid layout menggunakan CSS Grid -->
<!-- Responsif: 1 kolom di HP, 2 kolom di tablet, 4 kolom di laptop besar -->
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
  {#each dashboardStats as stat}
    
    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
      
      <!-- Icon Container (Warna latar belakang dan SVG berbeda-beda sesuai data) -->
      <div class={`w-10 h-10 ${stat.iconBg} rounded-lg flex items-center justify-center mb-6`}>
      ...
      <!-- Text Container -->
      <div>
        <!-- Angka tebal dan besar dengan warna hijau -->
        <h3 class={`text-4xl font-extrabold ${stat.valueColor} mb-1 tracking-tight`}>
          {stat.value}
        </h3>
        
        <!-- Label deskripsi -->
        <p class="text-[15px] font-bold text-gray-500">
          {stat.title}
        </p>
      </div>
